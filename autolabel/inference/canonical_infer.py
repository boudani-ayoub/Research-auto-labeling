...
def canonical_infer(model:     YOLO,
                    ...
                    conf:      float = 0.05,
                    iou_nms:   float = 0.45,
                    imgsz:     int   = 640) -> CandidatePool:
    """
    Run canonical inference on a list of images and return a CandidatePool.

    Canonical inference settings are identical across all rounds:
      - Fixed imgsz with letterbox padding
      - No augmentation
      - Fixed conf threshold (intentionally loose to populate C_t)
      - Fixed NMS iou threshold

    Args:
        model:     loaded YOLO model (ultralytics.YOLO)
        image_ids: list of image identifiers, one per image
        images:    list of uint8 BGR numpy arrays [H, W, 3]
    ...
    results = predictor(source=images, stream=False)

    pool: CandidatePool = {}

    for img_idx, (image_id, result) in enumerate(zip(image_ids, results)):
