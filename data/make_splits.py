...
def filtered_split_to_labeled_data(split: Dict) -> Dict:
    """
    Convert a FILTERED split dict (from make_filtered_coco_splits.py) to
    a labeled_data dict for YOLOTrainer.

    Difference from split_to_labeled_data():
      - Reads image_dir from `image_dir_train` (the symlink directory under
        the filtered output root, NOT the original COCO images directory).
        This ensures Ultralytics' /images/->/labels/ path substitution lands
        inside the filtered tree.
      - Reads label_dir from `label_dir_train` (filtered + remapped labels).
        Does not accept a label_dir override — the filtered labels are
        single-purpose and live where the split JSON says they live.
      - Filenames are still derived from labeled_train_ids /
        labeled_inner_val_ids / unlabeled_ids (same convention as standard
        splits — the filtered protocol shares image_id integers).

    Returned shape is identical to split_to_labeled_data() so the trainer's
    existing assembly logic (validators, copy loops, dataset.yaml writer)
    works without modification. The trainer doesn't care whether the split
    is filtered or not — it just consumes a labeled_data dict.

    Args:
        split: loaded split dict from load_split() pointing at a JSON
               produced by make_filtered_coco_splits.py.

    Raises:
        ValueError       if split is missing fields specific to the
                         filtered schema (image_dir_train, label_dir_train,
                         labeled_train_ids, labeled_inner_val_ids).
        FileNotFoundError if image_dir_train or label_dir_train do not
                         exist on disk (regenerate the filtered tree).
    """
    # Validate filtered schema fields
    required = ("labeled_train_ids", "labeled_inner_val_ids",
                "image_dir_train", "label_dir_train", "class_names")
    for field in required:
        if field not in split:
            raise ValueError(
                f"Filtered split is missing required field {field!r}.\n"
                f"This split was not generated by make_filtered_coco_splits.py "
                f"(or was generated by an outdated version). Regenerate with:\n"
                f"  python data/make_filtered_coco_splits.py "
                f"--coco_dir <coco_root> --output_dir <filtered_root> "
                f"--splits_dir <splits_dir>"
            )

    image_dir = Path(split["image_dir_train"])
    label_dir = Path(split["label_dir_train"])

    if not image_dir.exists():
        raise FileNotFoundError(
            f"Filtered image directory not found: {image_dir}\n"
            f"The symlinked filtered image tree is missing. Re-run "
            f"make_filtered_coco_splits.py to regenerate."
        )
    if not label_dir.exists():
        raise FileNotFoundError(
            f"Filtered label directory not found: {label_dir}\n"
            f"Re-run make_filtered_coco_splits.py to regenerate."
        )
    ...
    return {
        ...
        "image_list":           train_fnames,        # labeled-train (~90%)
        "inner_val_image_list": inner_val_fnames,    # inner-val (~10%, frozen)
        "unlabeled_image_dir":  str(image_dir),      # same dir — list-level check
        "unlabeled_list":       unlabeled_fnames,
    }
